...
from typing import Callable, Dict, Iterable, Iterator, List, Literal, Optional, Sequence, Tuple, Union
...
TokenSelector = Union[int, Literal["all"], Callable[[int], Sequence[int]]]
...
class AbstractSAE(nn.Module, ABC):
    ...
    def regularization_loss(
        ...
        if loss_mask is None:
            ...
            l2 = (features.pow(2) * mask).sum() / (valid_tokens * features.shape[-1])
        ...
        l2_coeff = float(getattr(getattr(self, "cfg", None), "l2_coefficient", 0.0))
        ...
        total = l1_coeff * l1 + l1_context_coef * context_l1 + l2_coeff * l2
        metrics = {
            "l1": float(l1.detach()),
        }
        if l2_coeff != 0.0:
            metrics["l2"] = float(l2.detach())
            metrics["l2_coef"] = l2_coeff
...
class TrainableSAE(AbstractSAE):
    ...
    def parameter_dtype(self) -> torch.dtype:
        """Return the dtype used by the SAE's learned weights."""
        return self.encoder.weight.dtype

    def pre_activations(self, x: torch.Tensor) -> torch.Tensor:
        x = x.to(device=self.encoder.weight.device, dtype=self.parameter_dtype)
    ...
    def decode(self, features: torch.Tensor) -> torch.Tensor:
        features = features.to(device=self.decoder.weight.device, dtype=self.parameter_dtype)
...
class SAEConnector:
    ...
    def features_for_prompt(
        ...
        projector_token_index: TokenSelector = -1,
    ...
    def _resolve_projector_indices(
        self,
        token_count: int,
        projector_token_index: TokenSelector,
    ) -> Sequence[int]:
        if projector_token_index == "all":
            return list(range(token_count))
        if callable(projector_token_index):
            indices = list(projector_token_index(token_count))
        else:
            indices = [int(projector_token_index)]
        ...
        for index in indices:
            ...
            if index < 0 or index >= token_count:
                raise IndexError(
                    f"projector_token_index={index} is out of range for sequence length {token_count}."
                )
    ...
    def _apply_projector_to_token(
        ...
        token_count = vectors.shape[1]
        indices = self._resolve_projector_indices(token_count, projector_token_index)

        for index in indices:
            token_vectors = projected[:, index, :]
            ...
            projected[:, index, :] = projected_vectors
        return projected
...
def load_hooked_transformer(
    ...
    **kwargs,
    ...
    resolved_device = resolve_device(device)
    requested_dtype = kwargs.get("torch_dtype", kwargs.get("dtype", None))
    if isinstance(requested_dtype, str):
        hooked_transformer_module = importlib.import_module("transformer_lens.HookedTransformer")
        DTYPE_FROM_STRING = hooked_transformer_module.DTYPE_FROM_STRING
        requested_dtype = DTYPE_FROM_STRING[requested_dtype]
