...
\begin{quote}
\small\ttfamily
...mountain lions, moose, and other big cats. And more than 200 free bears are critically endangered. But is it bad for wildlife or bad for Colorado critically? The Lucy Catt Family, a Pine County native, is one of the few mountain lions that escap[ed]...
\end{quote}

\noindent\textbf{ClampOp(25):}
\begin{quote}
\small\ttfamily
...mountain lions, moose, and kameks, both of which need to be brain induced. Once sharks and catfish have their heads bowed so you can catch these 25 sharks (30--40 years old) for 25 hectorie for five separate river sharks...
\end{quote}

\noindent\textbf{ClampOp(75):}
\begin{quote}
\small\ttfamily
...mountain lions, moose, and luxury creatures, yet neither bills embedded in the 6120-protisher-fin oil, pin fin and fin 2012 - the fourth season premiere...Sharks HC[...]
\end{quote}

\noindent\textbf{CondDistOp($\mathcal{N}(45, 10)$, $p=0.4$):}
\begin{quote}
\small\ttfamily
...mountain lions, moose, and wild river turtles. Cetacella uncertainica, a cetaceus encountered at enclosure loss by the ancient predators, are the source of this unusual event. The cetaceus---that everything otherworldly is far more typically consu[med]...
\end{quote}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{figures/method_comparison.png}
\caption{Feature 18493 activation over the generated sequence for four methods. Solid line: post-intervention value. Dashed line: natural value. \textit{[Screenshot the feature activation chart from the steering dashboard, showing baseline, clamp\_25, clamp\_75, and cond\_dist side by side or in sequence.]}}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{figures/param_effects.png}
\caption{Effect of $p$, $\mu$, and $\sigma$ on each of the three evaluation metrics. \textit{[Screenshot the parameter effects scatter/line plots from the grid search dashboard, showing all three metrics on the vertical axis against each parameter on the horizontal axis.]}}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{figures/heatmap.png}
\caption{Heatmap of shark-strength (color) as a function of $\mu$ (x-axis) and $p$ (y-axis), with $\sigma = 10$. The transition from low to high shark-strength occurs near $p = 0.3$--$0.4$ and $\mu = 40$--$60$. \textit{[Screenshot the $\mu \times p$ heatmap from the grid search dashboard, filtered to $\sigma = 10$.]}}
\end{figure}
